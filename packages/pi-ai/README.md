...
```bash
npm install @pilore/pi-ai
```

TypeBox exports are re-exported from `@pilore/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@pilore/pi-ai';
import { builtinModels } from '@pilore/pi-ai/providers/all';
...
```typescript
import { anthropicProvider } from '@pilore/pi-ai/providers/anthropic';
import { openaiProvider } from '@pilore/pi-ai/providers/openai';
import { openrouterProvider } from '@pilore/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@pilore/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@pilore/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@pilore/pi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@pilore/pi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@pilore/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@pilore/pi-ai';
...
```typescript
import { builtinImagesModels } from '@pilore/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@pilore/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@pilore/pi-ai';
...
```typescript
import { stream } from '@pilore/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@pilore/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@pilore/pi-ai';
import { anthropicProvider } from '@pilore/pi-ai/providers/anthropic';
import { openaiProvider } from '@pilore/pi-ai/providers/openai';
import { googleProvider } from '@pilore/pi-ai/providers/google';
...
```typescript
import { createModels } from '@pilore/pi-ai';
import { anthropicProvider } from '@pilore/pi-ai/providers/anthropic';
...
- `@pilore/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@pilore/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@pilore/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@pilore/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@pilore/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@pilore/pi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@pilore/pi-ai/bedrock-provider';
...
```bash
npx @pilore/pi-ai login              # interactive provider selection
npx @pilore/pi-ai login anthropic    # login to specific provider
npx @pilore/pi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@pilore/pi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@pilore/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@pilore/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@pilore/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@pilore/pi-ai`.
