# @pilore/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@pilore/pi-ai`.

## Installation

```bash
npm install @pilore/pi-agent-core
```
...
```typescript
import { Agent, streamProxy } from "@pilore/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@pilore/pi-agent-core";
