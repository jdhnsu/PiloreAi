...
```bash
npm install @pilore/pi-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@pilore/pi-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@pilore/pi-telemetry';
...
`@pilore/pi-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@pilore/pi-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@pilore/pi-agent-core';
...
- `@pilore/pi-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@pilore/pi-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@pilore/pi-agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
