...
export function parseCodeProfile(source: string, fileName: string): ProfileDefinition {
...
export function loadCodeProfiles(dir: URL | string = DEFAULT_DIR): ProfileDefinition[] { return readdirSync(dir).filter((file) => file.endsWith(".md")).sort().map((file) => parseCodeProfile(readFileSync(new URL(file, dir), "utf8"), file)); }
let defaults: ProfileDefinition[] | undefined; export function getDefaultCodeProfiles(): ProfileDefinition[] { return defaults ??= loadCodeProfiles(); }
